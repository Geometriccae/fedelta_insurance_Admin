...
class Leads extends CI_Controller
{
    ...
    public function quotes($id)
    {
        ...
        $this->load->view('scripts/quotes', $data);
    ...
    public function upload_document()
{
    ...
    if (!empty($_FILES['document2']['name'])) {
        ...
        // If no file is uploaded, proceed without file processing

        // Save data to database without file name
        $data = array(
            'insurance_type' => $insurance_type,
            'customer_id' => $lead_id,
            'sum_insured' => $this->input->post('sum_insured'),
        ...
        // Redirect or show success message
        redirect(base_url("leads/quotes/$lead_id")); 
        $this->session->set_flashdata('success', 'Data saved successfully.'); // Redirect to a specific page after successful save
    }
}
